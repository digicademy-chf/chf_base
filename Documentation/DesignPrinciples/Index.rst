...
2. Generic **classes that are reused** across dependent data models are stored
  in ``chf_base``. If a class would need lots of overrides if it were included
...
3. Models designed to store data sets should be held together by a **central
  ``Resource`` class** (e.g., ``BibliographicResource`` for a bibliography).
...
4. Another common pattern across the CHF is to allow the use of a **``SameAs``
  class and a ``Tag``**. The former helps identify entities in Linked Open
  Data scenarios through identifiers from authority files, and the latter
  offers generic and, in some cases, extensible keyword and label systems.
...
6. A data model extension **should contain** the TCA,
  ...
  the data model, generic Fluid templates for the frontend, a search
  configuration, and embedded metadata to make the views it provides as
